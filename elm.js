...
Elm.Counter.make = function (_elm) {
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([centerClass]),
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,Decrement)]),_U.list([$Html.text("- count")]))
...
Elm.Main.make = function (_elm) {
   ...
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $CounterPair = Elm.CounterPair.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   ...
   var _op = {};
   var main = $StartApp$Simple.start({model: A2($CounterPair.init,0,0),update: $CounterPair.update,view: $CounterPair.view});
